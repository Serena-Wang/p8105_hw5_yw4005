...
sim_normal_t_test <- function(mu) {
  ...
  test_result <- t.test(sample, conf.level = 1-0.05) %>%
    tidy() %>%
...
![](p8105_hw5_yw4005_files/figure-gfm/unnamed-chunk-12-1.png)<!-- --> As
the true mu increases, rejection rate grows as well. In other words,
power grows as effect size grows.
...
![](p8105_hw5_yw4005_files/figure-gfm/unnamed-chunk-13-1.png)<!-- --> As
the true mu increases, sample average mu hat increases as well. Average
mu hat of samples that reject the null are very close the true mu when
the true mu is 4, 5, 6. When the true mu is less than 4, mu hat of
samples that reject the null are larger than the true mu.
